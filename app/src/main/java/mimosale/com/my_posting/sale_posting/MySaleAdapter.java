...
public class MySaleAdapter extends RecyclerView.Adapter<MySaleAdapter.MyViewHolder> {
    List<MySaleModel> mySaleModelList;

    Context mctx;
    Shimmer shimmer;
ProgressBar p_bar;
    public MySaleAdapter( List<MySaleModel> mySaleModelList, Context mctx,ProgressBar p_bar) {
        this.mySaleModelList = mySaleModelList;
        this.mctx = mctx;
        shimmer = new Shimmer();
        this.p_bar=p_bar;
    }
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder holder, final int position) {
        final MySaleModel items = mySaleModelList.get(position);
    ...
    public void updateAction(final String sale_id, final int position, ImageView iv_pop_up)
    {
            ...
            public boolean onMenuItemClick(MenuItem item) {
                if (item.getItemId()==R.id.delete)
                {
                    new SweetAlertDialog(mctx, SweetAlertDialog.WARNING_TYPE)
                            ...
                            .setContentText(mctx.getResources().getString(R.string.want_to_delete_sale))
                            ...
                            .setConfirmClickListener(new SweetAlertDialog.OnSweetClickListener() {
                                ...
                                public void onClick(SweetAlertDialog sDialog) {
                                    ...
                                    deleteSalePosting(sale_id,position);
                                }
                            })
                ...
                {
                    Intent i=new Intent(mctx,SalePostingActivity.class);
                    i.putExtra("shop_id",mySaleModelList.get(position).getShop_id());
                    i.putExtra("sale_name",mySaleModelList.get(position).getName());
                    i.putExtra("desc",mySaleModelList.get(position).getDescription());
                    i.putExtra("price","");
                    i.putExtra("sale_id",mySaleModelList.get(position).getId());
                    i.putExtra("min_discount",mySaleModelList.get(position).getMin_discount());
                    i.putExtra("max_discount",mySaleModelList.get(position).getMax_discount());
                    i.putExtra("hash_tag",mySaleModelList.get(position).getHash_tags());
    ...
    public void deleteSalePosting(String sale_id, final int position)
    {
        ...
        try {
            ...
            service.delete_sale(PrefManager.getInstance(mctx).getUserId(),sale_id,"Bearer "+PrefManager.getInstance(mctx).getApiToken(),new Callback<JsonElement>() {
                @Override
                public void success(JsonElement jsonElement, Response response) {
                    ...
                    try {
                        ...
                        if (status.equals("1")) {
                            mySaleModelList.remove(position);
